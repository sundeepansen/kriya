...
def generate_sentence(chanda_info):
    ...
    for _ in range(chanda_info['sama_vruttams']):
        ...
        if len(' '.join(line + [word])) <= letters_per_line:
            line.append(word)
        else:
...
# Encode a character into the corresponding Chandas pattern sentence
def encode_char(char):
    chanda_number = random.randint(1, 26)
    chanda_info = chanda_patterns[chanda_number]
    sentence = generate_sentence(chanda_info)
    return sentence



# Encrypt a message using the Chandas patterns
def encrypt_message(message, encrypted_sentence=None):
    encrypted_message = []
    for char in message:
        if char.isalpha():  # Only encrypt alphabetic characters
            [encrypted_sentence].append(encode_char(char))
            encrypted_message.append(encrypted_sentence)
